...
typedef struct {
    ...
    uint32_t out_fps; //视频输出帧率,默认值为30
    uint32_t width;
    uint32_t height;
...
typedef struct {
    /* gop:I帧间隔 */
    int32_t gop_mode;       // 高级GOP模式(可填：VENC_GOP_MODE_E中的类型，位于rk_comm_venc.h头文件内，默认不需要配置(填0))
    int32_t gop_len;        // (0:[I帧间隔采用2*fps_out_num], n:[I帧间隔为n(1==n,即每一帧都是I帧)])
    /* fps_out:输出帧率, 输出帧率=(fps_out_num/fps_out_den)] */
    int32_t fps_out_num;    // (0:[使用默认值], n:[使用n值]) -- 默认值为WorkPara.out_fps
    int32_t fps_out_den;    // (0:[使用默认值], n:[使用n值]) -- 默认值为1
    /* fps_in :输入帧率, 输入帧率=(fps_in_num/fps_in_den)] */
    int32_t fps_in_num;     // (0:[使用默认值], n:[使用n值]) -- 默认值为WorkPara.out_fps
    int32_t fps_in_den;     // (0:[使用默认值], n:[使用n值]) -- 默认值为1
    /* bps:码率 */
    int32_t rc_mode;        // 码率控制模式(可填：VENC_RC_MODE_E中的类型，位于rk_comm_rc.h头文件内)
    int32_t fixQp;          // 固定质量(仅当rc_mode为[VENC_RC_***FIXQP]时生效, 取值范围[1,51]。不为[VENC_RC_***FIXQP]默认(填0))
